...
function [D2,dmaha] = maha(x,A,z)
  ...
  xeqz = isempty(z); dg = strcmp(z,'diag');           % sort out different modes
  n = size(x,1); m = size(z,1);                     % dimensions: number of rows
  if dg                                                            % vector d2xx
    D2 = zeros(n,1);
  else
    ...
    if xeqz
      mu = mean(x,1);% mean of all rows -> row vector
    else
    ...
    D22 = max(bsxfun(@plus,sax2,bsxfun(@minus,saz',2*x2*Az')),0);
    D2 = min(D2, D22);
  end
  if nargout>1
